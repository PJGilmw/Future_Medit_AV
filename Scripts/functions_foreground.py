...
def create_foreground_db(name_foreground,
    ...
    "Function that creates the foreground databases corresponding to the specific background"
...
def finalize_foreground_db(name_foreground,
    ...
    "Function that finalizes the foreground databases corresponding to the specific background"
    ...
    if not exists_already:
        ...
        for exc in list(photovoltaicslantedroofinstallation3kWpsingleSipanelmountedonroof_premise.exchanges()):
                ...
                if exc["name"]=='market for photovoltaic panel, single-Si wafer':
        ...
        for exc in list(photovoltaicslantedroofinstallation3kWpmultiSipanelmountedonroof_premise.exchanges()):
                ...
                if exc["name"]=='market for photovoltaic panel, multi-Si wafer':
        ...
        for exc in list(photovoltaicslantedroofinstallation3kWpCISpanelmountedonroof_premise.exchanges()):
                ...
                if exc["name"]=='market for photovoltaic panel, CIS':
        ...
        for exc in list(photovoltaicslantedroofinstallation3kWpCdtepanelmountedonroof_premise.exchanges()):
                ...
                if exc["name"]=='market for photovoltaic laminate, CdTe':
    ...
    else: #  We do not create the foreground but only collect necessay values
    ...
    # The parameters that are collected in the premise databases
    ...
    parameters_database = {"original_surface_panel_single":original_surface_panel_single,
                          ...
                          "new_efficiency_single":new_efficiency_single,
                          "new_efficiency_multi":new_efficiency_multi,
                          "new_efficiency_cis":new_efficiency_cis,
                          "new_efficiency_cdte":new_efficiency_cdte,
                          "lifetime":lifetime}     
...
def get_PV_efficiency_indatabase(PV_installation_activity):
    old_efficiency = PV_installation_activity["log parameters"]["old efficiency"]
    new_efficiency = PV_installation_activity["log parameters"]["new efficiency"]

    return old_efficiency,new_efficiency


def get_PV_yield_and_lifetime_indatabase(PV_production_activity):
    correctedyield = [param for param in PV_production_activity["parameters full"] if param["description"]=='Yield (Output), photovoltaics, corrected' ]
    correctedyield=correctedyield[0]["amount"]
    
    lifetime = [param for param in PV_production_activity["parameters full"] if param["description"]=='lifetime' ]
    lifetime = lifetime[0]["amount"]


    return correctedyield,lifetime
