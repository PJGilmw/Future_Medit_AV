...
def load_activities_dict_andfunctions(foregroundAVS,background_db,Biosphere_db,additional_biosphere_db,global_vars):
    ...
    "Called by the main script. Collects the activities in the databases, sets up the parameters dictionaries. Passes them to global to be used in the main script"
   ...
   "elec_wafer_rer_single":elec_wafer_rer_single,
    ...
    PV_par_distributions = {
        ...
        'concrete_mount_upd_PV': ['list', [1,1],"."], #first one is SIRTA
        
        'concrete_mount_ratio_avs_pv': ['list', [0,1],"."],   # first one is SIRTA no concrete
        
        'aluminium_mount_upd_PV': ['list', [0.38,0.38],"."], # Besseau used 1.5kg in his parasol file so it corresponds to a ratio of 0.38 with ecoinvent

        'aluminium_mount_ratio_avs_pv': ['list', [0,1],"."], # no alu in sirta
        
        'steel_mount_upd_PV': ['list', [0.46,0.46],"."],  # according to ratios computed with Besseau
            
        'steel_mount_ratio_avs_pv': ['list', [3.06,3.82],"."],  # 1.41 is the with the original ratio if we take the amount in ecoinvent so if we take the updated one  from Besseau it's 1.41/0.46 =  3.06  
    
        'poly_mount_upd_PV': ['list', [1,1],"."],
            
        'poly_mount_ratio_avs_pv': ['list',[1,1],"."],
    
    
        'corrbox_mount_upd_PV': ['list', [1,1],"."],
            
        'corrbox_mount_ratio_avs_pv': ['list', [1,1],"."],
        
        
        'zinc_mount_upd_PV': ['list', [0.46,0.46],"."],  # zinc for coating steel
            
        'zinc_mount_ratio_avs_pv': ['list', [3.06,3.82],"."],
...
def set_activity_exchanges_as_variables(activity,background_db,Biosphere,suffix, includeloc,includecat):
    ...
    """ Utility function that collects the input activitites and sets them as global variables."""
